...
var Record = function(name, recordOptions, connection, options, client) {
	...
	this._resubscribeNotifier = new ResubscribeNotifier(this._client, this._sendRead
		.bind(this));
	this._readAckTimeout = setTimeout(this._onTimeout.bind(this, C.EVENT.ACK_TIMEOUT),
		this._options.recordReadAckTimeout);
	this._readTimeout = setTimeout(this._onTimeout.bind(this, C.EVENT.RESPONSE_TIMEOUT),
...
Record.prototype.set = function(pathOrData, data) {
	...
	if (arguments.length === 2 && (typeof pathOrData !== 'string' || pathOrData.length ===
			0)) {
...
Record.prototype.subscribe = function(path, callback, triggerNow) {
	...
	if (args.path !== undefined && (typeof args.path !== 'string' || args.path.length ===
			0)) {
		throw new Error('invalid argument path');
	}
...
Record.prototype.discard = function() {
	...
	this.whenReady(function() {
		...
		if (this.usages <= 0) {
			...
			this._discardTimeout = setTimeout(this._onTimeout.bind(this, C.EVENT.ACK_TIMEOUT),
...
Record.prototype.delete = function() {
	...
	this.whenReady(function() {
		this.emit('destroyPending');
		this._deleteAckTimeout = setTimeout(this._onTimeout.bind(this, C.EVENT.DELETE_TIMEOUT),
			this._options.recordDeleteTimeout);
...
Record.prototype._$onMessage = function(message) {
	...
	} else if (message.action === C.ACTIONS.UPDATE || message.action === C.ACTIONS
		.PATCH) {
		this._applyUpdate(message, this._client);
	}
...
Record.prototype._recoverRecord = function(remoteVersion, remoteData, message) {
	console.log(
		'Record.prototype._recoverRecord remoteVersion, remoteData, message',
		remoteVersion, remoteData, message)
	message.processedError = true;
	if (this._mergeStrategy) {
		this._mergeStrategy(this, remoteData, remoteVersion, this._onRecordRecovered
			.bind(this, remoteVersion, remoteData));
	} else {
		this.emit('error', C.EVENT.VERSION_EXISTS, 'received update for ' +
			remoteVersion + ' but version is ' + this.version);
	}
};
 ...
 * @returns {void}
 */
Record.prototype._onRecordRecovered = function(remoteVersion, remoteData, error,
	data) {
	console.log('_onRecordRecovered remoteVersion, remoteData, error, data',
		remoteVersion, remoteData, error, data)
	if (!error) {
		...
		*/
		...
		if (this._mergeStrategy != this._client.MERGE_STRATEGIES.REMOTE_WINS)
...
Record.prototype._applyUpdate = function(message) {
	...
	this.version = version;
	this._applyChange(message.action === C.ACTIONS.PATCH ? message.data[2] :
...
Record.prototype._setReady = function() {
	...
	for (var i = 0; i < this._queuedMethodCalls.length; i++) {
		this[this._queuedMethodCalls[i].method].apply(this, this._queuedMethodCalls[
			i].args);
	}
	this._queuedMethodCalls = [];
	this.emit('ready');
...
Record.prototype._checkDestroyed = function(methodName) {
	if (this.isDestroyed) {
		this.emit('error', 'Can\'t invoke \'' + methodName + '\'. Record \'' + this.name +
